...
- `human_labels`: 10 个已冻结 Gold + 45 个 AI 辅助人工校正页 + 5 个盲标对照页
...
- 在 45 页显示可编辑 AI 初稿，在 5 页盲标对照中隐藏同页 AI 初稿；
- 自动累计前台复核时长，并要求设计师显式确认逐项复核。

## 每页操作顺序

1. 先检查截图是否仍有未发现的破图、主体缺失或严重布局错乱；发现后停止标注，
   记录 sample id，不自行更改 assignment；
2. AI 辅助页先核对已有原子元素，删除误检、合并拆散对象并补充漏检；盲标对照页
   仍从“节点”开始创建；
3. 在“实体”中逐项复核语义分组和设计树；AI 层级回放指标较低，不能默认接受；
4. 为每个 group 复核布局模式、gap、padding、对齐和 resize；
5. 在“Token”中逐项查看重复样式候选，只点击“保留”那些设计上应同步修改且至少
   有两个成员的关系；名称和精确值由系统自动填写；
6. 处理完正例后点击“完成 Token 检查”；没有正例也必须点击，以记录显式负例；
7. 勾选“设计师已完成逐项复核”，再检查右侧校验；
8. AI 辅助页点击“提交人工校正版”，盲标页点击“提交完成”；
9. 提交后确认样本前出现勾选，进度增加 1。

## 阶段门槛

1. 先完成 5 页 AI 辅助页，检查误检删除量、漏检补充量和单页复核时间；若校正
   成本接近从零标注，暂停并调整预标注范围；
2. 5 个盲标对照必须保持 AI 初稿隐藏，完成人工提交前不得切换为辅助模式；
3. 50 页完成后冻结 `annotator_a`，比较两种条件的复核时间、AI 相似度和修改量；
4. AI 辅助页只能称为“设计师校正的 AI 预标注”，不能称为独立人工盲标；
5. 每页经设计师确认后才能生成 `gold_intent.json`；
6. test 的 20 页在模型、阈值和排除规则冻结前不得用于调参。
...
测试固定检查样本数、split、重复簇、结构指纹、冻结状态、AI 初稿来源哈希、
盲标可见性、资源路径和 Token 富集最低配额。辅助标注的完整协议见
`docs/thesis/17_ai_assisted_annotation_protocol.md`。
