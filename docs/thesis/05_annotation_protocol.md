...
5. 在“Token”页签检查系统按重复样式生成的候选，只保留设计上应联动修改的关系，
   最后点击“完成 Token 检查”；只有系统漏掉明显关系时才使用“手动补充”；
...
- 从已创建实体的颜色、文字、圆角和布局间距中生成重复样式候选，并自动填充
  Token 名称与精确值；
- 保证每个实体最多保留一条父边；
...
Token 标注只回答一个问题：**这些对象的该项样式在设计修改时是否应当一起变化？**

- 保留：同类按钮的背景色、同层级标题的文字样式、同类卡片的圆角、同一列表中
  重复使用的间距；
- 忽略：浏览器继承造成的相同默认文字色、不同语义区域偶然相同的数值、仅有一个
  成员的样式、截图无法支持“应联动”判断的关系；
- 标注者不再手工抄写颜色值、字号、字重、圆角或间距，也无需命名 Token；这些值
  从 PageGraph 或已确认布局中自动写入；
- 若没有任何候选值得保留，直接点击“完成 Token 检查”。系统会记录显式负例，避免
  把空数组误解为尚未检查。
