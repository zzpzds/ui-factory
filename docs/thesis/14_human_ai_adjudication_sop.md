...
本流程用于当前无法取得第二位独立标注者时的辅助复核。标注者 A 是人工主标注，
`annotator_ai` 是在冻结前未查看人工标签的 AI 代理。该流程产生的是
**单人 AI 辅助复核/定稿的金标准**，不是独立裁决；不能写成“双人标注”“标注者
间一致性”或把 AI 计入真人标注员人数。
...
- `审核人标识`：使用稳定编号；不能填写 `annotator_ai`。公开数据不保留姓名，
  除非另有明确同意记录。
...
方法章节应明确说明：AI 代理在盲态下独立生成候选标注，用于发现分歧；最终草稿
以同一人的人工底稿初始化，再由该人员结合截图、HTML/PageGraph 和差异提示定稿，
因此存在初始化锚定，不是独立 adjudication。当前 7/10 审核说明只是简短摘要，
只能证明审核声明已提交，不能证明每条差异均有独立决策日志。人机指标属于跨来源
描述性比较，不是 inter-annotator reliability。条件允许时，应另请外部人员抽查
至少 2 页，并报告人员资历、作者关系、抽查范围和发现。
